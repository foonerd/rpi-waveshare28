...
Operator configuration is `waveshare28-config`. See
[`../docs/CONFIG.md`](../docs/CONFIG.md).

The renderer reads `/etc/waveshare28-panel.toml` (or a path as the first
argument). That file is generated by `apply` and overwritten on every run.
A missing file means the SKU 27579 defaults. A file that exists but does
